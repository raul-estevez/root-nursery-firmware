## ⚖️ License
This firmware is Open Source software. This project is committed to the principles of Open Source Hardware. We provide all design files, schematics, and source code to empower others to build, learn, and improve upon this hydroponics monitoring system. 

### Software License
The source code in this repository is licensed under the 
**[GNU General Public License v3.0 (GPL-3.0)](https://www.gnu.org/licenses/gpl-3.0.html)**.

**Under this license:**
* **Freedom to Run:** You can use this software for any purpose.
* **Freedom to Study & Modify:** You have full access to the source code to adapt it to your specific needs.
* **Copyleft (Stay Open):** If you distribute a modified version of this software, you **must** make your modified source code available under the same GPL-3.0 license. This prevents the software from being "locked away" in proprietary products.
* **No Warranty:** This software is provided "as is" in the hope that it will be useful, but without any warranty.

### Hardware Compatibility
This firmware is specifically designed to run on the **[Root Project] Hardware**, which is licensed under the **CERN-OHL-S v2** and can be found in the [Hardware Repository](LINK_TO_YOUR_HARDWARE_REPO).

